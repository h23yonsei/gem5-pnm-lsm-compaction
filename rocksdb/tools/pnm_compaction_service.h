...
//   Wait():     block on the socket for the compaction result;
//               pnm_compaction_unit sends it once the compaction is done and
//               MMIO STATUS shows the PNM latency model's delay has passed.
//
// The device models one job at a time, and Schedule() resets it before each
// submit. With db_bench's max_background_jobs=2, RocksDB runs one compaction
// at a time, so jobs never overlap on the device.
//
// MSHR safety: rocksdb_pnm runs on core 0 while pnm_compaction_unit does
// DRAM-heavy SST I/O on core 1.  Separate cores → separate L1 MSHRs → no
// cache-hierarchy conflict.
#pragma once
...
class PNMCompactionService : public ROCKSDB_NAMESPACE::CompactionService {
 public:
  ...
  ROCKSDB_NAMESPACE::CompactionServiceJobStatus Wait(
    ...
    // pnm_compaction_unit polls MMIO STATUS_DONE on core 1 before sending,
    // so this recv() unblocks only after both real compaction and the PNM
    // timing model have completed — core 0 sleeps here instead of spinning.
 ...
 private:
  ...
  // MMIO writes are release stores, a plain mov on x86: the region is mapped uncached, so the
  // device sees SRC, DST and CMD in program order. A seq_cst store compiles to xchg, a locked
  // read-modify-write that also reads the write-only CMD register, and gem5's TIMING CPU routes
  // that locked uncached access to DRAM instead of the device.

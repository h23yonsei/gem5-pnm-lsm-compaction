...
//   2. Pins this process to core 0 (the main CPU) before forking;
//      pnm_compaction_unit then pins itself to core 1.
...
int main(int argc, char** argv) {
  // Pin this process to core 0 (the main CPU), leaving core 1 to
  // pnm_compaction_unit. Its background flush threads inherit the pin.
  cpu_set_t cpuset;
  CPU_ZERO(&cpuset);
  CPU_SET(0, &cpuset);
  if (sched_setaffinity(0, sizeof(cpuset), &cpuset) != 0) {
    fprintf(stderr, "[rocksdb_pnm] sched_setaffinity core 0 failed (%s)\n",
            strerror(errno));
  } else {
    fprintf(stderr, "[rocksdb_pnm] pinned to core 0\n");
  }
  ...
  if (readlink("/proc/self/exe", exe_path, sizeof(exe_path) - 1) > 0) {
    ...
    if (g_pnm_unit_pid == 0) {
      ...
      // Give pnm_compaction_unit time to bind its socket. In the gem5 runs
      // this falls inside the measured region (run_pnm.py starts measuring
      // before launching rocksdb_pnm), so it adds 0.5 s to the PNM run's
      // simulated time; db_bench's own phase timings do not include it.
  ...
  // on core 1 via socket.  PNMCompactionService fires MMIO CMD_SUBMIT with

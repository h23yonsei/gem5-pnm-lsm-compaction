...
1. Pins itself to core 0 via `sched_setaffinity`, leaving core 1 to the worker. (The capstone
   code pinned it to cores 0–1, every core of the two-core PNM configuration; see the README's
   [Limitations of the capstone runs](../../README.md#limitations-of-the-capstone-runs).)
...
1. Pins itself to core 1 via `sched_setaffinity`. (The capstone code tried core 2, a layout
   for three cores; on the two-core PNM configuration the call failed and the worker ran
   unpinned.)
...
4. Each thread calls `handle_job()`:
   ...
   - Opens a persistent secondary DB via `DB::OpenAsSecondary()` on first call
     (`/tmp/pnm_secondary`); subsequent calls reuse it. It is opened with the primary's options
     and column families, loaded from the primary's latest OPTIONS file with
     `LoadLatestOptions()`, because `CompactWithoutInstallation()` writes its output with the
     secondary's own column-family options. The output compression is therefore db_bench's
     `--compression_type=none`, and the worker logs it at open. (The capstone code opened the
     secondary with default `Options`, so its output used RocksDB's default Snappy compression;
     see the README's
     [Limitations of the capstone runs](../../README.md#limitations-of-the-capstone-runs).)
   ...
   - Logs the job's input, output and replaced record counts and its bytes read and written.
...
| CPU cores | 1 (KVM → TIMING), or 2 with `PNM_BASELINE_CORES=2` | 2: `rocksdb_pnm` on core 0, the PNM worker on core 1 (not pinned in the capstone runs) |
| PNM device | — | `PNMCompactor` at `0xD0000000` |
| Kernel module | — | `pnm_module.ko` loaded before ROI |
| Output dir | `m5out/baseline` (`m5out/baseline_2core` with two cores) | `m5out/pnm` |
| CPU switch point | `workbegin`, or a checkpoint taken there and restored on TIMING CPUs | `workbegin` (after `m5.drain()`), or a checkpoint as for the baseline |
...
| `configs/yonsei/pnm_module.c` and `pnm_module.Makefile`, built inside the image by `chroot` | `/root/pnm_module.ko` |
